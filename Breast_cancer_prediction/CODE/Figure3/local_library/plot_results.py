...
# %%
def main(
        results, cond_agents, cond_trainEXs, order,
    ...
    # The arrays are 2D, with models (e.g. NN, SVM) on the rows
    # and distributed types (sub_models, e.g. central, agent, ensemble)
    # on the columns
    num_models = len(results.keys())
    num_sub = len(order)
    arrayplot = dict()
    for s in class_labels.keys():
        arrayplot[s] = keywise_template(
                keywise_template([], savedir_Heatmap),
                perf_metrics)

#    arrayplot = dict()
#    for s in sub_titles.keys():
#        arrayplot[s] = keywise_template(
#                keywise_template(
#                        keywise_template([], savedir_Heatmap),
#                        perf_metrics),
...
#    arrayplot = dict(
#            agent=keywise_template(
#                    keywise_template(
#                            keywise_template([], savedir_Heatmap),
#                            perf_metrics
#                    ),
#                    class_labels),
#            GEC=keywise_template(
#                    keywise_template(
#                            keywise_template([], savedir_Heatmap),
#                            perf_metrics
#                    ),
            ...
            for t in savedir_Heatmap.keys():
                for r in results.keys():
                    for s in order:
                        ...
                        arrayplot[j][k][t].append(
                                results[r][s][j][k][t])
                arrayplot[j][k][t] = np.concatenate(
                        arrayplot[j][k][t])
                arrayplot[j][k][t] = np.reshape(
                        arrayplot[j][k][t],
                        (num_models, num_sub))

                array_mins = list()
                array_maxs = list()
                array_mins.append(np.min(arrayplot[j][k][t]))
                array_maxs.append(np.max(arrayplot[j][k][t]))
...
#        xticks[0] = 'Central'
    ...
    for s in order:
        xticks.append(sub_titles[s])
#        xticks[i] = str(xticks[i]) + \
#            '\n(' + str(cond_trainEXs['values'][i]) + ')'

#    xlabel = str(cond_agents['values'][2]) + \
#        " agents with majority class 0  and  " + \
#        str(cond_agents['values'][3]) + \
#        " agents with majority class 1" + \
#        '\n' + \
#        '(Per agent:  ' + str(cond_trainEXs['values'][2]) + \
#        " examples from Majority class  and  " + \
#        str(cond_trainEXs['values'][3]) + \
#        " examples from Minority class)"
...
#    '''
#    dummy = np.zeros([num_models, num_sub])
...
#                    # Since it's a grid of nx2 columns, mod 0 makes it "left"
##                    if i % 2 == 0:
#                        # Plot the agent on the left
##                        dmod = 'agent'
#                sns.heatmap(
#                        # The data
#                        arrayplot[j_class][k_perf][t_type],
...
#                    # Since it's a grid of nx2 columns, mod 1 makes it "right"
##                    elif i % 2 == 1:
##                        # Plot the GEC on the right
##                        dmod = 'GEC'
##                        sns.heatmap(
##                                # The data
##                                arrayplot['GEC'][j_class][k_perf][t_type],
##                                # The axis on the subplot
##                                ax=ax,
##                                # Set min and max of colorbar (same for all)
##                                vmin=cbar_lim['min'][j_class][k_perf][t_type],
...
#                                   fontsize=axlabelsize, rotation=90)
