...
# %%
def average_g1_g2_performance(
        agent_predict_prob, agent_predict_class,
        num_agents_dict, group_1, group_2,
        targets, class_labels, perf_metrics
        ):
    """
    Get average agent results for agents in 2 groups
        For ex. "large" and "small" agents for large and small agents
        or "major_c0" and "minor_c0" for unbalanced agents

    num_agents_dict is the dictionary that stores the number of agents
    in total as well as in each group.

    agents in "group_1" (string that points to the key in num_agents_dict)
    are presumed to be the first set of results in "prob" and "class" lists
    """
    # %%
    agent_keys = [group_1, group_2]  # This order matters and must be kept
    ...
    return avg_results[group_1], avg_results[group_2]
