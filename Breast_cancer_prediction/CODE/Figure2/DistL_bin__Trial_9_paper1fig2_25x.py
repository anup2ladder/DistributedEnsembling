...
conditions_agents_filename = "conditions_agents_paper1_fig2_uniform.csv"
...
dist_titles = {
    'agent': "Average Individual Agent",
    'GEC_AP': "Distributed Ensemble (Average Probability)",
    'GEC_MV': "Distributed Ensemble (Majority Vote)"
}
...
num_train_agent = cond_trainEXs['values'][0]
...
num_test = num_examples - num_train_total
...
#                    )
        ...
        local_start = time.time()
        this_iteration['nn_low'] = {}
        this_iteration['nn_low']['agent'], _, \
            this_iteration['nn_low']['GEC_AP'], _, \
            this_iteration['nn_low']['GEC_MV'], _ = \
            models_NN.GEC_NN(
        ...
        local_start = time.time()
        this_iteration['nn_med'] = {}
        this_iteration['nn_med']['agent'], _, \
            this_iteration['nn_med']['GEC_AP'], _, \
            this_iteration['nn_med']['GEC_MV'], _ = \
            models_NN.GEC_NN(
                    'both AP and MV (this does not do anything)',
        ...
        local_start = time.time()
        this_iteration['nn_high'] = {}
        this_iteration['nn_high']['agent'], _, \
            this_iteration['nn_high']['GEC_AP'], _, \
            this_iteration['nn_high']['GEC_MV'], _ = \
            models_NN.GEC_NN(
                    'both AP and MV (this does not do anything)',
        ...
        kernel = 'linear'
        local_start = time.time()
        this_iteration['svm_lin'] = {}
        this_iteration['svm_lin']['agent'], _, \
            this_iteration['svm_lin']['GEC_AP'], _, \
            this_iteration['svm_lin']['GEC_MV'], _ = \
            models_SVM.GEC_SVM(
                    'both AP and MV (this does not do anything)',
                    kernel,
                    num_agents, agent_features, agent_targets,
        ...
        kernel = 'poly'
        local_start = time.time()
        this_iteration['svm_poly2'] = {}
        this_iteration['svm_poly2']['agent'], _, \
            this_iteration['svm_poly2']['GEC_AP'], _, \
            this_iteration['svm_poly2']['GEC_MV'], _ = \
            models_SVM.GEC_SVM(
                    'both AP and MV (this does not do anything)',
                    kernel,
                    num_agents, agent_features, agent_targets,
        ...
        kernel = 'poly'
        local_start = time.time()
        this_iteration['svm_poly3'] = {}
        this_iteration['svm_poly3']['agent'], _, \
            this_iteration['svm_poly3']['GEC_AP'], _, \
            this_iteration['svm_poly3']['GEC_MV'], _ = \
            models_SVM.GEC_SVM(
                    'both AP and MV (this does not do anything)',
                    kernel,
                    num_agents, agent_features, agent_targets,
        ...
        kernel = 'rbf'
        local_start = time.time()
        this_iteration['svm_rbf'] = {}
        this_iteration['svm_rbf']['agent'], _, \
            this_iteration['svm_rbf']['GEC_AP'], _, \
            this_iteration['svm_rbf']['GEC_MV'], _ = \
            models_SVM.GEC_SVM(
                    'both AP and MV (this does not do anything)',
                    kernel,
                    num_agents, agent_features, agent_targets,
        ...
        kernel = 'sigmoid'
        local_start = time.time()
        this_iteration['svm_sig'] = {}
        this_iteration['svm_sig']['agent'], _, \
            this_iteration['svm_sig']['GEC_AP'], _, \
            this_iteration['svm_sig']['GEC_MV'], _ = \
            models_SVM.GEC_SVM(
                    'both AP and MV (this does not do anything)',
                    kernel,
                    num_agents, agent_features, agent_targets,
        ...
        n_trees = 1
        this_iteration['rf_1'] = {}
        this_iteration['rf_1']['agent'], _, \
            this_iteration['rf_1']['GEC_AP'], _, \
            this_iteration['rf_1']['GEC_MV'], _ = \
            models_RF.GEC_RF(
                    'both AP and MV (this does not do anything)',
                    n_trees,
                    num_agents, agent_features, agent_targets,
        ...
        local_start = time.time()
        n_trees = 10
        this_iteration['rf_10'] = {}
        this_iteration['rf_10']['agent'], _, \
            this_iteration['rf_10']['GEC_AP'], _, \
            this_iteration['rf_10']['GEC_MV'], _ = \
            models_RF.GEC_RF(
                    'both AP and MV (this does not do anything)',
                    n_trees,
                    num_agents, agent_features, agent_targets,
        ...
        local_start = time.time()
        n_trees = 100
        this_iteration['rf_100'] = {}
        this_iteration['rf_100']['agent'], _, \
            this_iteration['rf_100']['GEC_AP'], _, \
            this_iteration['rf_100']['GEC_MV'], _ = \
            models_RF.GEC_RF(
                    'both AP and MV (this does not do anything)',
                    n_trees,
                    num_agents, agent_features, agent_targets,
        ...
        local_start = time.time()
        n_trees = 1000
        this_iteration['rf_1000'] = {}
        this_iteration['rf_1000']['agent'], _, \
            this_iteration['rf_1000']['GEC_AP'], _, \
            this_iteration['rf_1000']['GEC_MV'], _ = \
            models_RF.GEC_RF(
                    'both AP and MV (this does not do anything)',
                    n_trees,
                    num_agents, agent_features, agent_targets,
        ...
        local_start = time.time()
        n_trees = 10000
        this_iteration['rf_10000'] = {}
        this_iteration['rf_10000']['agent'], _, \
            this_iteration['rf_10000']['GEC_AP'], _, \
            this_iteration['rf_10000']['GEC_MV'], _ = \
            models_RF.GEC_RF(
                    'both AP and MV (this does not do anything)',
                    n_trees,
                    num_agents, agent_features, agent_targets,
            ...
            for d_dist in this_iteration[m_model].keys():
                for j_class in class_labels.keys():
                    for k_perf in perf_metrics:
                        iterations[m_model][d_dist][j_class][k_perf][i_it, ag]\
            ...
            for j_class in class_labels.keys():
                for k_perf in perf_metrics:
                    iter_result = models_train_test.mean_stdev_iterations(
                            iterations[i_model][d_dist][j_class][k_perf][:, ag]
                            )
                # iter_result is a dict with keys 'mean' and 'stdev'

                    for r in iter_result.keys():
                        results[i_model][d_dist][j_class][k_perf][r][ag] = \
                            iter_result[r]
...
summary_parameters = [
    ...
    ['Number of of training examples', num_train_total],
...
if save_output is True:
    ...
    # dist_titles.pickle
    data_in_out.pickle_out_variable(
            dist_titles, 'dist_titles.pickle', results_folder_relpath
            )
    ...
    shutil.copy2(
            os.path.join(conditions_foldername, conditions_agents_filename),
            os.path.join(results_folder_relpath, 'conditions_agents.csv')
            )
