...
A set of functions to train random forests with distributed learning
"""
...
def GEC_RF(
    ...
    """
    Global ensemble classifier using random forests to train agents
    ...
    central_weights, central_results, _ignore_scram, \
        ...
        central_predict_class, _ignore_scram = distributed_RF(
                n_trees,
    ...
    # Get all 7 returned values
    agent_RFs, avg_agent_results, scram_avg_agent_results, \
        agent_predict_prob, scram_predict_prob, \
        agent_predict_class, scram_predict_class = distributed_RF(
                n_trees,
    ...
    # %% Get results for average large and small agent
    avg_large_results, avg_small_results = \
        distributed_models.average_large_small_performance(
                agent_predict_prob, agent_predict_class,
                num_agents, test_targets, class_labels, perf_metrics
                )

    # %% Now ensemble the individual agents
    # with type of ensemble specified by the caller, with "gec_type"

    # Ensemble large agents
    ensemble_large = distributed_models.GEC_main(
            gec_type,
            agent_predict_prob[0:num_agents['large']],
            agent_predict_class[0:num_agents['large']],
            test_targets, class_labels, perf_metrics)

    # Ensemble small agents
    ensemble_small = distributed_models.GEC_main(
            gec_type,
            agent_predict_prob[num_agents['large']:num_agents['total']],
            agent_predict_class[num_agents['large']:num_agents['total']],
            test_targets, class_labels, perf_metrics)

    # Ensemble all agents, unweighted
    ensemble_all_unweighted = distributed_models.GEC_main(
            gec_type,
            agent_predict_prob,
            agent_predict_class,
            test_targets, class_labels, perf_metrics)

    # Ensemble all agents, weighted
    ensemble_all_weighted = distributed_models.GEC_weighted(
            gec_type, training_guide,
            agent_predict_prob,
            agent_predict_class,
            test_targets, class_labels, perf_metrics)

    # %% Return average agent and ensemble results
    return central_results, avg_agent_results, avg_large_results, \
        avg_small_results, ensemble_large, ensemble_small, \
        ensemble_all_unweighted, ensemble_all_weighted
...
def distributed_RF(
    ...
    # Test features
    agent_predict_prob, agent_predict_class = individual_agent_predictions_RF(
                agent_RFs, num_agents,
                test_features, classification_threshold=0.5
                )

    # Scrambled Test features
    scram_predict_prob, scram_predict_class = individual_agent_predictions_RF(
                agent_RFs, num_agents,
    ...
    return agent_RFs, avg_agent_results, scram_avg_agent_results, \
        agent_predict_prob, scram_predict_prob, \
...
def train_agents_RF(
    ...
    Returns a list of trained RFs, one per agent
    '''
...
# %%
def individual_agent_predictions_RF(
        agent_RFs, num_agents,
    ...
    for n_agent in range(0, num_agents):
        agent_predict_prob[n_agent], agent_predict_class[n_agent] = \
             model_predictions_RF(agent_RFs[n_agent], features)
...
# %%
def model_predictions_RF(RF_model, features):
    """
    Predict probabilities and classifications from RF(RFC) model
    The class predictions use RF.predict, and are cool
    The probabilities are hacky and limited to binary
        This is because I have to figure out which column of probabilities
        correspond to the classifications made with RF.predict().
        This is becaise I can't pin down necessarily which column corresponds
        to the predictions of RF.predict(), as this seems to vary sometimes

        What I essentially do then manually check which column to use
    """

    # Make normal RF predictions
    predict_class = RF_model.predict(features)
    ...
    # and sum to 1
    # Get row sum
    row_sum = np.sum(rf_proba, axis=1)
    # Scale each row by its sum
    for i in range(len(row_sum)):
        rf_proba[i, :] = rf_proba[i, :] / row_sum[i]
    ...
    rf_proba_class1 = models_train_test.classify_prob(rf_proba[:, 1])
    correspondance = sklearn.metrics.accuracy_score(
            predict_class, rf_proba_class1)
    if correspondance >= 0.5:
        ...
        predict_prob = rf_proba[:, 1]
    elif correspondance < 0.5:
        # Case where first column is representative of probabilities
        predict_prob = rf_proba[:, 0]
    else:
        assert False, "ERROR in predicted probabilities from RF"
